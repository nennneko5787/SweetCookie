...
class SpecValidateTaskTest {
    ...
    fun `implemented without a conformance case is rejected`() {
        ...
        expectFailure(
            SpecFixture(tmp).normative("SC-140").shardOf(
                "filters", "SC-140",
                SpecFixture.Entry("is_family", "implemented", impl = "net.nennneko5787.sweetcookie.x.C"),
            ),
            "no conformance case",
        )
    }
    ...
    fun `a stub claiming an implementation is rejected`() {
        expectFailure(
            SpecFixture(tmp).normative("SC-140").shardOf(
                "filters", "SC-140",
                SpecFixture.Entry("is_family", "stub", impl = "net.nennneko5787.sweetcookie.x.C"),
            ),
    ...
    fun `every problem is reported in one run, exactly once each`() {
        ...
        val fixture = SpecFixture(tmp).normative("SC-140").shardOf(
            "filters", "SC-140",
            SpecFixture.Entry("a", "implemented", impl = "net.nennneko5787.sweetcookie.x.A"),
            SpecFixture.Entry("b", "stub", impl = "net.nennneko5787.sweetcookie.x.B"),
