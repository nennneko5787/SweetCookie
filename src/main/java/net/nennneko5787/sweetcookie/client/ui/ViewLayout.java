...
public final class ViewLayout {
     ...
     *
     * <p>No wrapping and no scrolling yet: both need a font's measurements, which is a second thing
     * for a backend to provide, and neither is needed to make the development loop in SC-280 §1
     * usable. A view longer than the screen is a real limitation and is stated rather than hidden.
     */
        ...
        for (ViewModel.Section section : view.sections()) {
            ...
            for (ViewModel.Row row : section.rows()) {
                ...
                if (!row.detail().isEmpty()) {
                    lines.add(new Line(row.detail(), MARGIN_X + 8, y + LINE_HEIGHT, DETAIL));
                    y += LINE_HEIGHT;
                }
                y += LINE_HEIGHT;
