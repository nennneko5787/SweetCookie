...
import net.nennneko5787.lepus.core.registry.LedgerJson;
...
public final class WorldLedger {
    ...
    public static void save() {
        ...
        try {
            ...
            System.out.println("[Lepus] failed to write the ledger at " + directory
                    + ": " + failed);
        }
    }
    ...
    private static void load(Path worldData, SlotPool registered) {
        ...
        try {
            ...
            System.out.println("[Lepus] SCE-4014 ledger unreadable at " + directory
    ...
    private static void reportSlotsOutsideThePool(
        ...
        registered.shortfallAgainst(ledger.requiredPool()).forEach((sizeClass, needed) ->
                System.out.println("[Lepus] SCE-4013 " + directory
                        + " has bindings outside the registered block pool. Set"
                        + " lepus.blockPool." + sizeClass + " = " + needed
                        + " (currently " + registered.capacity(sizeClass)
                        + ") in config/lepus.json and restart. Those blocks stay bound and"
